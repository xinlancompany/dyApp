...
var openOutlink = function(url, title, closeId) {
	var userInfoStr = _get("userInfo", false);
	var userInfo = null;
	if (!!userInfoStr) {
	...
	if (!!userInfo) {
		_replaceAjax({
			...
			if (!!d.data) title = d.data;
			_openOutlink(url, title, closeId);
		});
	} else {
		_openOutlink(url, title, closeId);
	}
...
var _openOutlink = function(url, title, closeId) {
			...
			buttons: [{
				...
				onclick: function() {
					plus.nativeUI.actionSheet({
			        	...
			        	var img = "http://develop.wifizs.cn/static/zsdyPR/img/logo.png";
			        	if(e.index == 1) return share('outLink', url, title, img, 'weixin', 'WXSceneSession');
			        	if(e.index == 2) return share('outLink', url, title, img, 'weixin', 'WXSceneTimeline');
			        	if(e.index == 3) return share('outLink', url, title, img, 'qq', null);
			        })
				}
			}]
		},
				...
				s.send({
					...
					if (userInfoStr) {
					    ...
					    _getTodayScore(userInfo, function(score) {
                            ...
                            _scoreAjax({
                                cmd: "fetch",
                                sql: "select count(*) as cnt from scoreShareCourse where (userId = ? or idno = ?) and logtime = ?",
                                vals: _dump([userInfo.id, userInfo.idNo, _today()])
                            }, function(d) {
                                if (d.success && d.data) {
                                    if (d.data[0].cnt < 5) {
                                        _scoreAjax({
                                            cmd: "exec",
                                            sql: "insert into scoreShareCourse(userId, idno, url) values(?,?,?)",
                                            vals: _dump([userInfo.id, userInfo.idNo, hrefUrl])
                                        }, function(d) {
                                            if (d.success) {
                                                _set("score", _dump({
                                                    score: score,
