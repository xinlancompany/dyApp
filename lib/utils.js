				...
				s.send({
					...
					if (userInfoStr) {
					    ...
					    _getTodayScore(userInfo.id, function(score) {
                            ...
                            _scoreAjax({
                                cmd: "fetch",
                                sql: "select count(*) as cnt from scoreShareCourse where userId = ? and logtime = ?",
                                vals: _dump([userInfo.id, _today()])
                            }, function(d) {
                                if (d.success && d.data) {
                                    if (d.data[0].cnt < 5) {
                                        _scoreAjax({
                                            cmd: "exec",
                                            sql: "insert into scoreShareCourse(userId, url) values(?,?)",
                                            vals: _dump([userInfo.id, hrefUrl])
                                        }, function(d) {
                                            if (d.success) {
                                                _set("score", _dump({
                                                    score: score,
