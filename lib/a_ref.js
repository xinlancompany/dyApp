...
var __getScore = function(uinfo, cb) {
    var score = 0,
        today = _today();
    _scoreAjax({
        cmd: "multiFetch",
        ...
        _callAjax({
            cmd: "fetch",
            sql: "select ifnull(sum(e.credit), 0) as score from courseEnroll e, courses c where e.userId in (select id from user where idno = ?) and e.courseId = c.id and c.ifValid > 0 and e.logtime > ?",
