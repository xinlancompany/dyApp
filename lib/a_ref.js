...
var __getScore = function(uid, cb) {
    var score = 0,
        today = _today();
    _scoreAjax({
        cmd: "multiFetch",
        multi: _dump([
            {
                key: "login",
                sql: "select * from scoreDailyLogin where userId = "+uid+" and logtime = '"+today+"'"
            },
            {
                key: "share",
                sql: "select count(*) as cnt from scoreShareCourse where userId = "+uid+" and logtime = '"+today+"'"
            },
            {
                key: "news",
                sql: "select count(*) as cnt from newsEnroll where userId = "+uid+" and logtime = '"+today+"'"
            },
        ]) ,
    }, function(d) {
        if (d.success && d.data) {
        ...
        _callAjax({
            cmd: "fetch",
            sql: "select ifnull(sum(e.credit), 0) as score from courseEnroll e, courses c where e.userId = ? and e.courseId = c.id and c.ifValid > 0 and e.logtime > ?",
            vals: _dump([uid, today+" 00:00:00"])
        }, function(d) {
            if (d.success && d.data && d.data.length) {
                score += parseInt(d.data[0].score);
            }
