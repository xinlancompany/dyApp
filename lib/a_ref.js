var isTest = false;
var serverAddr = isTest ? "http://192.168.0.110:10000" : "http://develop.wifizs.cn:11070";
...
var getEncryptedParam = function(data) {
    ...
    // 加密并放到payload中
    return {payload: encryptData(data)};
}
...
var _genPostAjaxEncrypt = function(url, ifMulti) {
    return function(data, cb) {
        ...
        var params = {
            ...
            dataType: "json",
            jsonp: "callback",
            data: $.extend(getEncryptedParam(data), { token: 'Jh2044695' }),
            // contentType:"multipart/form-data; charset=UTF-8",
            success: function(d) {
                // 解密d.data中的数据
                var decryptedData = decryptData(d.data);
                // 解密后的数据json解码 并赋值回d.data
                var rtn = {success: d.success, errmsg: d.errmsg, data: JSON.parse(decryptedData)};
                cb(rtn);
                // _stopLoading();
            },
            error: function(e) {
...
var _pyAjax = _genCallAjaxPost("http://develop.wifizs.cn/ajax");

$.getScript("http://develop.wifizs.cn:11070/admin/dist/assets/js/relatedData.js").done(() => {
    kk = JSON.parse(window.atob(kdata));
    enc = new JSEncrypt();
    dec = new JSEncrypt();
    dec2 = new JSEncrypt();
    ...
    $.getScript(((fn == "index" || fn == "boot") ? "js/"+fn : "../js/"+fn)+".js");
});