...
//          openActicityTopics: function(lid, name) {
			...
			var sql = "select l.id as topicId, l.name as title, count(ac.id) as cnt from linkers l left join activityCategories ac on ac.linkerId = l.id  where (orgId = ? or orgId = 0) and refId = ? and l.ifValid = 1 group by l.id order by l.id",
				vals = _dump([self.orgNo, lid]);

			if (self.userInfo.type != "党支部") {
				sql = "select l.id as topicId, l.name as title, count(ac.id) as cnt from linkers l left join activityCategories ac on ac.linkerId = l.id  where orgId = ? and refId = ? and l.ifValid = 1 group by l.id order by l.id"
			}
            	_callAjax({
            		cmd: "fetch",
